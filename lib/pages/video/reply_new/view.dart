...
class _ReplyPageState extends CommonRichTextPubPageState<ReplyPage> {
  ...
  late final Rx<Account> _selectedAccount = () {
    if (Pref.enableReplyAccountSwitch && Pref.rememberReplyAccount) {
      final lastMid = Pref.lastReplyAccountMid;
      if (lastMid != null) {
        final found = Accounts.account.values.firstWhereOrNull(
          (a) => a.mid == lastMid,
        );
        if (found != null) {
          return found.obs;
    ...
    return Accounts.main.obs;
  }();

  @override
  void initState() {
    super.initState();
    if (Pref.enableReplyAccountSwitch && Accounts.account.length > 1) {
      for (final acc in Accounts.account.values) {
        Accounts.fetchProfile(acc.mid).then((_) {
          if (mounted) setState(() {});
        });
      }
    }
